/* This program implements a 4 digit up/down/random counter on 4 dgit 7seg LED display */
...
/*
BBB Expansion header P8 pins    GPIO number        4-digit         7seg display segment
-----------------------------------------------------------------------------------------------------------
P8.7                             GPIO_66               11                 a
P8.8                             GPIO_67               7                  b
P8.9                             GPIO_69               4                  c
P8.10                            GPIO_68               3                  h   //decimal point
P8.11                            GPIO_45               2                  d
P8.12                            GPIO_44               1                  e
P8.14                            GPIO_26               10                 f
P8.16                            GPIO_46               5                  g
...
/*
BBB Expansion header P9 pins    GPIO number        4-digit         7seg display segment
-----------------------------------------------------------------------------------------------------------
P8.15                             GPIO_48               12                 digit 1
P8.23                             GPIO_49               9                  digit 2
P8.30                             GPIO_112              8                  digit 3
P8.27                             GPIO_115              6                  digit 4
*/


/* uncomment this macro only if you use COMMON CATHODE display */
 ...
 */
void display_numbers(uint32_t number)
{
...
void start_updowncounting(int delay_value)
{
    ...
    while (1)
    {
        // Display the number without incrementing for delay_value cycles
        for (i = 0; i < delay_value; i++)
        {
            display_numbers(number);
        }

        // If counting up, increment; if counting down, decrement
        if (counting_up)
        {
            number++;
            if (number > 9999) // Reached max value, switch to counting down
            {
                counting_up = 0;
                number = 9999;
            }
        ...
        {
            number--;
            if (number == 0) // Reached min value, switch to counting up
            {
                counting_up = 1;
            }
        }
...
void start_randomcounting(int delay_value)
{
    ...
    while(1)
    {
        ...
        // Display the number without changing it for delay_value cycles
        for (i = 0; i < delay_value; i++)
        {
            display_numbers(number);
        }
