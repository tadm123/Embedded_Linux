/* This program controls the BBB USER LED 3 trigger brightness values */
...
/* This is the path corresponds to USER LED 3 in the 'sys' director */
//#define SYS_FS_LEDS_PATH    "/sys/class/leds/beaglebone:green:usr3"

#define USR_LED_NUMBER 3
#define SOME_BYTES 100

/* This function writes the trigger values for the given "led_no"
...
int main( int argc, char *arv[] )
{
    ...
    {
        printf("usage: %s <contorl option> <value>\n", argv[0]);
        printf("valid control options: brightness, trigger\n");
        printf("valid 'brightness' values: 0, 1\n");
        printf("valid 'trigger' values: heartbeat, timer, none, oneshot, default-on\n");
    }
    else {
        /* argc is valid, let's check argv */
        if (strcmp(argv[1], "trigger") == 0) {
            process_trigge_values(argv[2]);
        } else if (strcmp(argv[1], "brightness") == 0) {
            /* Converting argv[2] which is a brightness value to integer */
